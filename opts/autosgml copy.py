...
class common_sets():
    ...
    def __init__(self, lpf:LPF, p, 
                ...
                betas, beta_o, beta_d, epow, batch_scaler,
    ...
    def grp_devdt(self, device, dtype):
        ...
        self.dev_bscaler = self.batchscaler*fone
    ...
    def lr_compute(self, autolr, step, rpl, m_t, w_t, lr_avga, w_str, w_str2, a_t=1):# -> Tensor | Any | List[Tensor]:
        ...
        if autolr==True:
            ...
            if not self.lpf.tensor_lists:
                ...
                # average
                wstrn_t, w_str[rpl] = self.lpf.compute(in_t=wg_t, x=w_str[rpl], beta=self.dev_ssbeta, step=step, mode=self.ssmode, epp=self.epp)
                # average
                wstrd_t, w_str2[rpl] = self.lpf.compute(in_t=m_t[rpl], x=w_str2[rpl], beta=self.dev_ssbeta, step=step, mode=self.ssmode, epp=self.epp)
                                
                # optional
                ct = m_t[rpl].mul(a_t)
                wlcl = (wstrn_t.mul(ct)).div(wstrd_t.mul(ct).add(self.dev_eps))
                # print(wlcl)
                #
                ...
                # average
                lrat, lr_avga[rpl] = self.lpf.compute(in_t=ewg_t, x=lr_avga[rpl], beta=self.dev_ssbeta, step=step, mode=self.ssmode, epp=self.epp)  
                ...
                # average
                wstrn_t, wstrpl = self.lpf.compute(in_t=wg_t, x=wstrpl, beta=self.dev_ssbeta, step=step, mode=self.ssmode, epp=self.epp)
                # average
                wstrd_t, wstrpl2 = self.lpf.compute(in_t=m_t[rpl], x=wstrpl2, beta=self.dev_ssbeta, step=step, mode=self.ssmode, epp=self.epp)
                ...
                # average
                lrat, lrarpl = self.lpf.compute(in_t=ewg_t, x=lrarpl, beta=self.dev_ssbeta, step=step, mode=self.ssmode, epp=self.epp)                  
...
class AutoSGM(Optimizer):
    ...
    def __init__(self, params, *, lr_init=1e-3, spe=1, 
                ...
                beta_i=0.9, beta_e=0.999, beta_a=6, epow=2,
                eps=1e-8, weight_decay=0, 
                beta_o=0, beta_d=0,
                levels=1, dfac=0.001, batch_scaler=1,
        ...
        defaults = dict(p=levels,lr_init=lr_init,betas=(beta_i,beta_e,beta_a),
                        beta_o=beta_o,
                        beta_d=beta_d, epow = epow, dfac=dfac, batch_scaler=batch_scaler,
    ...
    def _init_group(self, group):
        ...
        if 'com_sets' not in group or group['com_sets'] is None:
            ...
            group['com_sets'] = common_sets(
                ...
                group['betas'], group['beta_o'], group['beta_d'], group['epow'], group['batch_scaler'],
        ...
        for p in group['params']:
            if p.grad is not None:
                ...
                if len(state)==0:
                    ...
                    dfac = float(group['dfac']) # or 0.001, 0.01
...
def _single_tensor_sgm(com_sets:common_sets, 
    ...
    for i, param in enumerate(params):
        ...
        for rpl in range(levels-1, -1, -1):
            ...
            com_sets.smooth_avg_out(step, rpl, w_t, w_smth)
    ...
    for (device, dtype) in grouped_tensors:
        ...
        for rpl in range(levels-1, -1, -1):
            ...
            if not device_has_sparse_grad:
                ...
                # [optional] smooth-averaging of the state
                com_sets.smooth_avg_out(step_this, rpl, dev_wt, dev_wt_smth)
