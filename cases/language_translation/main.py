...
def main(opts):
    ...
    # Set up logging
    fnme = opts.logging_dir+"cases/language_translation2/" + str(date.today()) 
    os.makedirs(fnme, exist_ok=True)
    logger = logging.getLogger(__name__)
    
    logging.root.handlers = []   
    # logging.basicConfig(filename=fnme + "/log.txt", level=logging.INFO) 
    logging.basicConfig(stream=sys.stdout, level=logging.INFO,)   
    
    fh = logging.FileHandler(filename=fnme + "/log.txt", encoding='ascii')
    ...
    # optim = AutoSGM(model.parameters(), 
    #                     autolr=False, lr_init=opts.lr, 
    #                     eps=1e-9, eps_mode=1,
    #                     beta_cfg=(0.9,0,0.999,0.999),)
